-- Add transactional RPC for placing orders with points
-- This ensures atomic order creation + points deduction with no race conditions
...
BEGIN
  ...
  -- This ensures no race conditions where a user could overspend their balance
  ...
  -- Validate all items and calculate total points
  ...
  LOOP
    ...
    -- If variant specified, validate and get adjustment
  ...
  -- Insert order items (recalculate to ensure consistency)
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    SELECT * INTO v_product
    FROM products
...
'Atomically creates an order with items and deducts points from user balance. Uses advisory lock to prevent race conditions. Runs as authenticated user with RLS.';
