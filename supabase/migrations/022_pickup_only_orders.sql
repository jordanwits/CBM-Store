-- Storefront is pickup-only: constrain orders and place_points_order accordingly

UPDATE orders SET delivery_method = 'pickup' WHERE delivery_method IS DISTINCT FROM 'pickup';

ALTER TABLE orders DROP CONSTRAINT IF EXISTS orders_delivery_method_check;
ALTER TABLE orders ADD CONSTRAINT orders_delivery_method_check CHECK (delivery_method = 'pickup');
...
'Pickup-only: atomically creates an order, decrements variant inventory, and deducts points.';
